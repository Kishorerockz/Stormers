...
### Ranjan — Diff Engine + AI Risk Scoring
...
### Vasanth — Backend API, Auth, RBAC, Audit Log
...
- Wire Kishore's snapshot function and Ranjan's diff function into the scheduler/endpoint flow
- **Deliverable by hour 3:** full API running locally, testable via Postman/curl

### Nithiesh — Frontend Dashboard
...
## 6. Git Workflow (agree on this in the first 30 min too)

- One shared GitHub repo, created by whoever has admin rights to set it up fastest. Add all 4 as collaborators immediately.
- `main` branch stays deployable at all times — never push broken code directly to `main`.
- Each person works on their own branch, named after their piece:
  - `kishore/snapshot-engine`
  - `ranjan/diff-ai-scoring`
  - `vasanth/backend-api-rbac`
  - `nithiesh/frontend-dashboard`
- **Push early, push often** — commit and push at least every 30-45 min, even half-working code. Don't sit on local changes for hours; if your laptop dies or you get stuck, the team needs to see where you left off.
- Merge to `main` as soon as your deliverable checkpoint (hour 3) is done, not all at once at the end. Whoever finishes first merges first, so later merges can build on what's already in `main`.
- If two people touch the same file (likely: Vasanth's API routes + whoever's calling them), talk before merging — don't let git resolve that silently.
- Once you're in the Live Game Window, remember: **the repo goes public and everything in it — code, commit history, issues — is visible to every other team.** Don't commit `.env` files, API keys, or credentials. Use `.env.example` with placeholder values and add real `.env` to `.gitignore` from commit #1.
...
| 3:00–3:45 | Integration: wire snapshot (Kishore) → diff (Ranjan) → alert/API (Vasanth) → frontend (Nithiesh) end-to-end. Fix contract mismatches |
...
## 8. README Must Include (per rulebook requirements)
...
## 9. Where You're Likely to Get Attacked in Phase 2 (defend these proactively)
